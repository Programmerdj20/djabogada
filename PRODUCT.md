...
- Activos disponibles: una foto de perfil de estudio (`img/fotoperfil.png`), un retrato de oficina para la página de perfil (`src/assets/images/daniela-perfil.webp`, corrección cálida de pared aplicada), imágenes de trabajo aportadas por el cliente para el resto del sitio (`src/assets/images/daniela-hero.webp`, `daniela-hero-plate.webp`, `daniela-defensa.webp`, `daniela-proceso.webp`, `daniela-contacto.webp`, esta última con corrección cálida de pared aplicada) y su hoja de vida en PDF. No hay casos, ni reseñas, ni contenido digital previo: todo el copy y contenido inicial debe ser autorado a nivel de producción por el equipo de desarrollo, marcado internamente como reemplazable donde dependa de material real futuro (sesión fotográfica, dirección, reseñas). Las imágenes de trabajo son material ilustrativo: no documentan casos ni diligencias concretas, y cualquier rótulo legible que traigan incrustado se excluye del encuadre (ver `DESIGN.md`).
...
- Imágenes de trabajo aportadas por el cliente, ya integradas vía `src/assets/images/` (optimizadas con sharp): `daniela-hero` (hero del home en móvil, a pantalla completa) y `daniela-hero-plate` (el mismo hero en desktop, placa enmarcada con passe-partout — ambas exclusivas de esa página; el hero no lleva titular ni texto superpuesto, solo foto y accesos de contacto), `daniela-defensa` (servicios, asesoría penal estratégica — ya no se usa en el home: la sección "¿Qué te está pasando?" es un índice de texto sin foto, para no repetir la composición de foto-sticky-con-columna de `/perfil`), `daniela-contacto` (contacto) y `daniela-proceso` (proceso penal, defensa penal — corrección cálida de pared aplicada, igual que `daniela-perfil` y `daniela-hero-plate`). Son ilustrativas; cualquier rótulo legible que traigan incrustado se recorta fuera de cuadro. `daniela-perfil` es un retrato de oficina distinto, aportado específicamente para la página de perfil, con la misma corrección cálida de pared que `daniela-hero-plate`. `daniela-escritorio` y `daniela-expediente` fueron eliminadas del repositorio por la clienta; `servicios/representacion-de-victimas.astro` y `servicios/criminalidad-organizada.astro` todavía las importan y quedan con el build roto hasta que esas páginas se aborden en un siguiente turno de trabajo.
